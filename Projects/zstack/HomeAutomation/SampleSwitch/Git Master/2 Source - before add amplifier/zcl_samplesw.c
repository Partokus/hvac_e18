...
uint8 response_buffer_on_off_button[25];
//uint8 response_buffer_on_off_button_len;

uint8 response_buffer_damper_button[25];
//uint8 response_buffer_damper_button_len;

uint8 response_buffer_pid_button[25];
//uint8 response_buffer_pid_button_len;

uint8 response_buffer_mode_button[25];
//uint8 response_buffer_mode_button_len;

uint8 response_buffer_cond_and_breezer_buttons[25];
//uint8 response_buffer_cond_and_breezer_buttons_len;

uint8 response_buffer_humidifier_buttons[25];
//uint8 response_buffer_humidifier_buttons_len;

uint8 response_buffer_temp_buttons[25];
//uint8 response_buffer_temp_buttons_len;

uint8 response_buffer_common_len; // will equal 25 for HVAC and 21 for AIRNANNY
...
uint16 zclSampleSw_event_loop( uint8 task_id, uint16 events )
{
      ...
      switch ( MSGpkt->hdr.event )
      {
        ...
        case ZDO_STATE_CHANGE:
          ...
          (zclSampleLight_NwkState == DEV_END_DEVICE) )
          {
            ...
            NLME_SetPollRate(1000); // set poll rate 1 sec temporary
  ...
  if ( events & SEND_GET_REQUEST )
  {
    ...
    can_send_data = 0;
    osal_start_timerEx( zclSampleSw_TaskID, SEND_GET_REQUEST, SEND_GET_REQUEST_DELAY );
    osal_start_timerEx( zclSampleSw_TaskID, SAMPLESW_TOGGLE_TEST_EVT, 1000 );
  ...
  /* PRESSED BUTTONS */
  if( events & SEND_MESSAGE_BY_ON_OFF_BUTTON_EVT )
  { 
    if (can_send_data == 1) SendData( &zclSampleSw_DstAddr, COMMAND_SEND, response_buffer_common_len, response_buffer_on_off_button );
    else osal_start_timerEx( zclSampleSw_TaskID, SEND_MESSAGE_BY_ON_OFF_BUTTON_EVT, 500 );
    //atmeex_led_toggle();
    return (events ^ SEND_MESSAGE_BY_ON_OFF_BUTTON_EVT);
  }
  
  if( events & SEND_MESSAGE_BY_DAMPER_BUTTON_EVT )
  { 
    if (can_send_data == 1) SendData( &zclSampleSw_DstAddr, COMMAND_SEND, response_buffer_common_len, response_buffer_damper_button );
    else osal_start_timerEx( zclSampleSw_TaskID, SEND_MESSAGE_BY_DAMPER_BUTTON_EVT, 500 );
    //atmeex_led_toggle();
    return (events ^ SEND_MESSAGE_BY_DAMPER_BUTTON_EVT);
  }
  
  if( events & SEND_MESSAGE_BY_PID_BUTTON_EVT )
  { 
    if (can_send_data == 1) SendData( &zclSampleSw_DstAddr, COMMAND_SEND, response_buffer_common_len, response_buffer_pid_button );
    else osal_start_timerEx( zclSampleSw_TaskID, SEND_MESSAGE_BY_PID_BUTTON_EVT, 500 );
    //atmeex_led_toggle();
    return (events ^ SEND_MESSAGE_BY_PID_BUTTON_EVT);
  }
  
  if( events & SEND_MESSAGE_BY_MODE_BUTTON_EVT )
  { 
    if (can_send_data == 1) SendData( &zclSampleSw_DstAddr, COMMAND_SEND, response_buffer_common_len, response_buffer_mode_button );
    else osal_start_timerEx( zclSampleSw_TaskID, SEND_MESSAGE_BY_MODE_BUTTON_EVT, 500 );
    //atmeex_led_toggle();
    return (events ^ SEND_MESSAGE_BY_MODE_BUTTON_EVT);
  }
  
  if( events & SEND_MESSAGE_BY_COND_AND_BREEZER_BUTTONS_EVT )
  { 
    if (can_send_data == 1) SendData( &zclSampleSw_DstAddr, COMMAND_SEND, response_buffer_common_len, response_buffer_cond_and_breezer_buttons );
    else osal_start_timerEx( zclSampleSw_TaskID, SEND_MESSAGE_BY_COND_AND_BREEZER_BUTTONS_EVT, 500 );
    //atmeex_led_toggle();
    return (events ^ SEND_MESSAGE_BY_COND_AND_BREEZER_BUTTONS_EVT);
  }
  
  if( events & SEND_MESSAGE_BY_HUMIDIFIER_BUTTONS_EVT )
  { 
    if (can_send_data == 1) SendData( &zclSampleSw_DstAddr, COMMAND_SEND, response_buffer_common_len, response_buffer_humidifier_buttons );
    else osal_start_timerEx( zclSampleSw_TaskID, SEND_MESSAGE_BY_HUMIDIFIER_BUTTONS_EVT, 500 );
    //atmeex_led_toggle();
    return (events ^ SEND_MESSAGE_BY_HUMIDIFIER_BUTTONS_EVT);
  }
  
  if( events & SEND_MESSAGE_BY_TEMP_BUTTONS_EVT )
  { 
    if (can_send_data == 1) SendData( &zclSampleSw_DstAddr, COMMAND_SEND, response_buffer_common_len, response_buffer_temp_buttons );
    else osal_start_timerEx( zclSampleSw_TaskID, SEND_MESSAGE_BY_TEMP_BUTTONS_EVT, 500 );
    //atmeex_led_toggle();
    return (events ^ SEND_MESSAGE_BY_TEMP_BUTTONS_EVT);
  }
...
static void zclSampleLight_OnOffCB( uint8 cmd )
{
  ...
  if (NLME_GetShortAddr() ==  pPtr->srcAddr.addr.shortAddr) return; // if it own address
  //atmeex_led_toggle();
  //if (pPtr->srcAddr.addr.shortAddr == 0x00) SendData( &zclSampleSw_DstAddr, COMMAND_SEND, (pPtr->cmd.DataLength - 3), &(pPtr->cmd.Data[3]) );
  
    if (pPtr->cmd.Data[3] == 0x02 && pPtr->cmd.Data[4] == 0xA8 && pPtr->cmd.Data[7] != 0x45) // if it OTA format packet and it is not firmaware's version check command
    {
      //atmeex_led_toggle();
      ...
      //uint8 ackbuf[9] = {0xFF, 0xEE, 0xDD, 0xCC, 0xBB, 0xAA, 0x99, 0x88, (pPtr->cmd.DataLength - 3)};
      //SendData( &zclSampleSw_DstAddr, COMMAND_SEND, 9, ackbuf ); 
      ...
      RxOnIdle = TRUE;
      ZMacSetReq( ZMacRxOnIdle, &RxOnIdle ); 
      NLME_SetPollRate( 1000 ); // make poll rate faster
    ...
    } else
    {
      ...
      {
        osal_start_timerEx(zclSampleSw_TaskID, ATMEEX_SET_DEFAULT_POLL_RATE_ON_START, POLL_RATE_FOR_MOBILE_APP_DELAY); // set poll rate 1 second temporary
        NLME_SetPollRate( 1000 ); // make poll rate faster
    ...
    //ATMEEX_POWER_SAVING_MODE_TURN_ON();
}
...
static void ATMEEX_send_data_over_air0( void )
{
 /* 0x70 0x65 PANID PANID  N      DATA       LRC1   LRC2
  *  [0]  [1]  [2]   [3]  [4]  [5, 6.. 3+N] [N+4]  [N+5]
  *
 ...
 */
  ...
  {
    uint8 buffer_len;
    buffer_len = MySerialApp_Tx0Buf[4] + 6; // calculate sending buffer lenght
    
    if (MySerialApp_Tx0Len > buffer_len) // if in buffer is more bytes than was calculate
    {
        switch (MySerialApp_Tx0Buf[buffer_len]) // look up what button was pressed
        {
        case ON_OFF_BUTTON:
          response_buffer_common_len = buffer_len;
          osal_memcpy(response_buffer_on_off_button, MySerialApp_Tx0Buf, buffer_len);
          osal_start_timerEx( zclSampleSw_TaskID, SEND_MESSAGE_BY_ON_OFF_BUTTON_EVT, SEND_MESSAGE_BY_BUTTONS_DELAY );
          break;
          
        case DAMPER_BUTTON:
          response_buffer_common_len = buffer_len;
          osal_memcpy(response_buffer_damper_button, MySerialApp_Tx0Buf, buffer_len);
          osal_start_timerEx( zclSampleSw_TaskID, SEND_MESSAGE_BY_DAMPER_BUTTON_EVT, SEND_MESSAGE_BY_BUTTONS_DELAY );
          break;
          
        case PID_BUTTON:
          response_buffer_common_len = buffer_len;
          osal_memcpy(response_buffer_pid_button, MySerialApp_Tx0Buf, buffer_len);
          osal_start_timerEx( zclSampleSw_TaskID, SEND_MESSAGE_BY_PID_BUTTON_EVT, SEND_MESSAGE_BY_BUTTONS_DELAY );
          break;
          
        case MODE_BUTTON:
          response_buffer_common_len = buffer_len;
          osal_memcpy(response_buffer_mode_button, MySerialApp_Tx0Buf, buffer_len);
          osal_start_timerEx( zclSampleSw_TaskID, SEND_MESSAGE_BY_MODE_BUTTON_EVT, SEND_MESSAGE_BY_BUTTONS_DELAY );
          break;
          
        case COND_AND_BREEZER_BUTTONS:
          response_buffer_common_len = buffer_len;
          osal_memcpy(response_buffer_cond_and_breezer_buttons, MySerialApp_Tx0Buf, buffer_len);
          osal_start_timerEx( zclSampleSw_TaskID, SEND_MESSAGE_BY_COND_AND_BREEZER_BUTTONS_EVT, SEND_MESSAGE_BY_BUTTONS_DELAY );
          break;
          
        case HUMIDIFIER_BUTTONS:
          response_buffer_common_len = buffer_len;
          osal_memcpy(response_buffer_humidifier_buttons, MySerialApp_Tx0Buf, buffer_len);
          osal_start_timerEx( zclSampleSw_TaskID, SEND_MESSAGE_BY_HUMIDIFIER_BUTTONS_EVT, SEND_MESSAGE_BY_BUTTONS_DELAY );
          break;
          
        case TEMP_BUTTONS:
          response_buffer_common_len = buffer_len;
          osal_memcpy(response_buffer_temp_buttons, MySerialApp_Tx0Buf, buffer_len);
          osal_start_timerEx( zclSampleSw_TaskID, SEND_MESSAGE_BY_TEMP_BUTTONS_EVT, SEND_MESSAGE_BY_BUTTONS_DELAY );
          break;
          
        default:
          SendData( &zclSampleSw_DstAddr, COMMAND_SEND, buffer_len, MySerialApp_Tx0Buf ); // if somehow it was initiated not by pressed button       
          break;         
        }      
    } else {
